...
def pingpong(n):
    ...
    True
    """
    def helper(k, direction, tally):
        if k == n:
            return tally + direction
        elif k % 8 == 0 or num_eights(k):
            return helper(k+1, -direction, tally + direction)
        else:
            return helper(k+1, direction, tally + direction)
...
def count_coins(change):
    ...
    True
    """
    def change_helper(change, small_coin):
        #base case
        if small_coin == None:
            return 0
        if change == 0:
            return 1
        if small_coin > change:
            return 0
    
        # do not include the smallest coin
        without_coin = change_helper(change, next_larger_coin(small_coin))
        # do include the smallest coin
        with_coin = change_helper(change-small_coin, small_coin)
